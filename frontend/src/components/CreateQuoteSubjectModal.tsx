...
interface CreateQuoteSubjectModalProps {
  isOpen: boolean
  onClose: () => void
  onSuccess: (subject: QuoteSubject) => void
}

export default function CreateQuoteSubjectModal({
  isOpen,
  onClose,
  onSuccess,
}: CreateQuoteSubjectModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null)
    try {
      const created = await QuoteSubjectsAPI.create({
      ...
      setError(err.response?.data?.detail || err.message || 'שגיאה ביצירת הפרויקט')
    } finally {
      setSaving(false)
    }
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      ...
      >
        <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700">
          <h2 className="text-xl font-bold text-gray-900 dark:text-white">הוספת פרויקט</h2>
          <button
            type="button"
            onClick={handleClose}
